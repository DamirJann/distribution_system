...
AllHistory aggregate_history_from_c_processes(struct pipe_table pipe_table, struct log_files log_files, local_id local_pid) {
    ...
    while (received_balance_msg_count != pipe_table.size - 1) {
        ...
        switch (msg.s_header.s_type) {
            ...
            default:
                fprintf(stderr, "%d: %d process got message with UNKNOWN type(%hd) during WORK step\n",
                        get_lamport_time(), PARENT_ID, msg.s_header.s_type);
                break;
        }
...
void start_k_process(local_id local_pid, struct log_files log_files, struct pipe_table pipe_table) {
    ...
    fprintf(log_files.event_log, log_received_all_ack_fmt, get_lamport_time(), local_pid);
    fprintf(stdout, log_received_all_ack_fmt, get_lamport_time(), local_pid);
...
void transfer(void *parent_data, local_id src, local_id dst,
    ...
    if (receive_specific_message(dst, &process_info, ACK) != 0){
        fprintf(stderr, "[%d]: k-process didn't get ack message", get_lamport_time());
    }
